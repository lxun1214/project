...
function findThrowPosterAgo() {
	caps.asyncPostJson("gift/listGiftCode.do", {
		pageNo: $("#pageNo").val(),
...
function getResultPageThrowPosterAgo(result) {
	...
	$.each(result.list, function(index, one) {
		var tr = $("<tr></tr>");
		$(tr).append("<td style='text-align: center;width:6%;'><nobr>"+one.serverName+"</nobr></td>");
		$(tr).append("<td style='text-align: center;width:6%;'><nobr>"+one.id+"</nobr></td>");
		$(tr).append("<td style='text-align: center;width:6%;'><nobr>"+one.giftName+"</nobr></td>");
		$(tr).append("<td style='text-align: center;width:13%;'><nobr> <span class='tooltip_gdts' data-jbox-title='礼包道具' data-jbox-content='"+one.itemId+"' >"+one.itemId+"</span ></nobr></td>");
		$(tr).append("<td style='text-align: center;width:6%;'><nobr>"+one.codeCount+"</nobr></td>");
		
		
		$(tr).append("<td style='text-align: center;width:9%;'><nobr><span class='label label-info'>" + new Date(one.starTime).format('yyyy-MM-dd hh:mm:ss') + "</span></nobr></td>");
		$(tr).append("<td style='text-align: center;width:9%;'><nobr><span class='label label-info'>" + new Date(one.endTime).format('yyyy-MM-dd hh:mm:ss') + "</span></nobr></td>");
		
		$(tr).append("<td style='text-align: center;width:6%;'><nobr>"+one.adminName+"</nobr></td>");
		...
		$(tr).append("<td style='text-align: center;width:13%;'><nobr><a class='link-onlian' id='link-onlian' href='Javascript:downloadExcel(" + one.id + ");'  >　下载　</a></td>");
...
function downloadExcel(id) {
	confirmx("确认要下载查看当前礼包码？<span style='color:red;'>（为了防止恶意下载，系统限定每日只能下载一次）</span>",function() {
		window.open("excel/createExcelGiftCode.do?giftCodeId="+id);
	});
}

function toAddMall(id){
	$.jBox.open("iframe:sellersys/mall/toAddMall/"+id+".do","添加商品",600,400,{ buttons: { '关闭': true} });
}
...
<div class="searchHead">
	...
	<form id="searchForm" class="breadcrumb form-search" action="#" method="post">
		...
		<ul class="ul-form" >
				<li><label style="width:80px;">服务器：</label>
		  	<select id="serverId" name="serverId" style="width:150px;height: 35px;margin-top: 5px"  class="input-xlarge">
			   <c:forEach items="${servers}" var="st">
			       <option value="${st.serverId}">${st.name}</option>
			   </c:forEach>
			</select>
		      </li>		
