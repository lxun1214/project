...
function findThrowPosterAgo() {
	caps.asyncPostJson("sellersys/users/listUser.do", {
		pageNo: $("#pageNo").val(),
...
function getResultPageThrowPosterAgo(result) {
	...
	$.each(result.list, function(index, one) {
		...
		$(tr).append("<td style='text-align: center;width:13%;'><nobr><span class=''>" + one.loginName + "</span ></nobr></td>");
		$(tr).append("<td style='text-align: center;width:13%;'><nobr><span class='tooltip_gdts' data-jbox-title='姓名' data-jbox-content='" + one.name + "'>" + one.name + "</span ></nobr></td>");
		$(tr).append("<td style='text-align: center;width:13%;'><nobr><span class='tooltip_gdts' data-jbox-title='部门' data-jbox-content='" + one.department + "'>" + one.department + "</span ></nobr></td>");
		$(tr).append("<td style='text-align: center;width:13%;'><nobr><span class='tooltip_gdts' data-jbox-title='联系方式' data-jbox-content='" + one.mobile + "'>" + one.mobile + "</span ></nobr></td>");
		if(one.channelId==0){
			$(tr).append("<td style='text-align: center;width:13%;'><nobr><span>所有渠道</span ></nobr></td>");
		}else if(one.channelId==1){
			$(tr).append("<td style='text-align: center;width:13%;'><nobr><span>官方渠道</span ></nobr></td>");
		}else if(one.channelId==2){
			$(tr).append("<td style='text-align: center;width:13%;'><nobr><span>甲壳虫渠道</span ></nobr></td>");
		}else{
			$(tr).append("<td style='text-align: center;width:13%;'><nobr><span>未知</span ></nobr></td>");
		}
		
		$(tr).append("<td style='text-align: center;width:9%;'><nobr><span class='label label-info'>" + new Date(one.lastLoginTime).format('yyyy-MM-dd hh:mm:ss') + "</span></nobr></td>");
		$(tr).append("<td style='text-align: center;width:13%;'><nobr><a class='link-onlian' id='link-onlian' href='Javascript:deleteUser(" + one.id + ");'  >　删除　</a><nobr><a class='link-onlian' id='link-onlian' href='Javascript:updatePwd(" + one.id + ")' >　重置密码　</a><a class='link-details' id='link-details' href='Javascript:updateUserInfo(" + one.id + ")'>编辑　</a><a class='link-details' id='link-details' href='Javascript:updateUserJurisdiction(" + one.id + ")'>配置权限　</a></nobr></td>");
		$("#tbody").append(tr);
	});
	$(".pagination").empty();
